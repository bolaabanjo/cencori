...
Indexed chunks persist in your project's store, so later requests can query the same document without re-uploading it — pass `file_search` with no file items and the model retrieves from everything you've indexed.
