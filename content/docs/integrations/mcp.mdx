...
| **Write** | `CENCORI_MCP_WRITE=1` | Inference (`generate_text`, `generate_rag`, `create_embeddings`, `moderate_content`, `generate_image`, vision, documents); memory (`remember_memory`, `write_memory`, `create_namespace`); agents (`create_agent`, `update_agent`); sessions (`create_session`, `add_session_turn`) |
