...
(static analysis, for `eline dev` / `eline build`) and **loads** them into an
...
*directories*, each a small agent of its own (see [Subagents](/eline/docs/subagents)).
...
`eline dev` and `eline build` run discovery first and report diagnostics. **Any
...
`eline build` serializes a summary of this to `.eline/manifest.json`. See the
[CLI reference](/eline/docs/cli).
