...
Zett executes agents against the **Cencori Sessions API**. The `eline/runner`
...
```ts
import { runAgent } from "eline/runner"; // also exported from "eline"
...
`streamAgent` is an async generator that yields [protocol events](/eline/docs/events)
as the turn unfolds:

```ts
import { streamAgent } from "eline/runner";
...
`eline dev` wraps `streamAgent` in a small HTTP server so you can exercise an agent
...
- `{ "message": "…", "stream": true }` → newline-delimited [event](/eline/docs/events) stream.
- `{ "message": "…" }` (no `stream`) → runs the turn and returns `{ "status": "ok" }`.

## Reading runtime state

Inside tool and hook handlers, the `eline/context` subpath exposes the ambient
