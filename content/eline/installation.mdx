...
- A **Cencori account** and project API key for running agents — see [Cencori integration](/eline/docs/cencori).
...
```bash
npx eline@latest init my-agent --template default
```
...
<Callout type="info">
`CENCORI_API_KEY` is required at run time. Without it, `eline dev` and `runAgent`
throw `Cencori API key required`. See [Cencori integration](/eline/docs/cencori).
</Callout>

Next: the [Quickstart](/eline/docs/quickstart) walks through your first request end to end.
