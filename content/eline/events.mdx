...
description: The typed StreamEvent protocol that runAgent, streamAgent, and eline dev emit.
---
...
`StreamEvent` union, defined in `eline/protocol`.
...
| --- | --- |
| `session.started` | `sessionId`, `runtime?` (`agentId`, `modelId`, `elineVersion`) |
...
`eline/protocol` exports constructor helpers and encoders — useful when you build a
...
```ts
import { encodeEvent, encodeEvents } from "eline/protocol";
