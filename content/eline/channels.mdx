...
import { defineChannel } from "eline/channels";
...
```ts
import { defineChannel, POST } from "eline/channels";
...
<Callout type="info">
During local development, `eline dev` exposes a single built-in HTTP endpoint
(`POST /`) regardless of your channels — see [Running agents](/eline/docs/running).
