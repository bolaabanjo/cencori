...
- A **Cencori account** and project API key for running agents — see [Cencori integration](/arcie/docs/cencori).
...
<Callout type="info">
`CENCORI_API_KEY` is required at run time. Without it, `arcie dev` and `runAgent`
throw `Cencori API key required`. See [Cencori integration](/arcie/docs/cencori).
</Callout>
