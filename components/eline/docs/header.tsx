...
import { useElineDocs } from "./sidebar-context";

// Slim top bar for the Eline docs — wordmark, sidebar filter, GitHub link, and a
// mobile nav toggle. Deliberately minimal and distinct from the Cencori docs header.
export function ElineDocsHeader() {
  const { query, setQuery, mobileOpen, setMobileOpen } = useElineDocs();
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-border/30 bg-background/80 backdrop-blur-sm">
      <div className="flex h-12 items-center gap-3 px-4 sm:px-6">
        ...
        >
          eline<span className="text-[#a855f7]">.</span>
        </Link>
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <a
            href="https://github.com/cencori/eline"
            target="_blank"
            rel="noreferrer"
            className="text-muted-foreground transition-colors hover:text-foreground"
            aria-label="Eline on GitHub"
          >
