...
const VO_IMG = 'https://raw.githubusercontent.com/cencori/cencori/redesign-signup-flow/public/vo.png';
...
function buildHtml(firstName: string, unsubscribeUrl: string): string {
...
<p style="margin:0 0 24px;font-size:14px;line-height:1.6;color:#555;">Voice is live on Cencori — text-to-speech and speech-to-text across six providers through one endpoint.</p>
<img src="${VO_IMG}" width="560" alt="" style="display:block;width:100%;height:auto;margin-bottom:24px;border-radius:8px;">
...
<tr>
<td align="center" style="background-color:#111;border-radius:6px;">
<a href="https://cencori.com/pricing" style="display:inline-block;padding:12px 24px;font-size:14px;color:#fff;text-decoration:none;font-weight:500;">Upgrade to Pro</a>
</td>
</tr>
</table>
<p style="margin:0 0 24px;font-size:14px;line-height:1.6;color:#555;">Or jump straight in:</p>
...
async function sendEmail(to: string, html: string, listUnsubscribeUrl?: string) {
  const body: Record<string, unknown> = {
    from: FROM,
    to,
    subject: 'Voice is live on Cencori',
    html,
  };
...
async function main() {
  ...
  for (let i = 0; i < allUsers.length; i += concurrency) {
    const slice = allUsers.slice(i, i + concurrency);
    const results = await Promise.allSettled(
      slice.map(async (u) => {
        ...
        const html = buildHtml(firstName, unsubscribeUrl);
